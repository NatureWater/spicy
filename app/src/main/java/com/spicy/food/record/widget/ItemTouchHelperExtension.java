...
public class ItemTouchHelperExtension extends RecyclerView.ItemDecoration
	...
	private static final int ACTION_MODE_SWIPE_MASK = ACTION_MODE_IDLE_MASK <<
	...
	private SwipeAcitonInterface mSwipActionInterface;
	...
	public boolean closeOpenedPreItem() {
		final View view = mCallback.getItemFrontView(mPreOpened);
		if (mPreOpened == null || view == null || mSelected == mPreOpened) {
			return true;
		}
		ObjectAnimator objectAnimator = ObjectAnimator.ofFloat(view, "translationX", view
				.getTranslationX(), 0f);
	...
	private void doChildClickEvent(float x, float y) {
		...
		if (consumeEventView != null) {
			//点击删除时 才可有点击事件
			if (mSwipActionInterface != null && mSwipActionInterface.getDeleteId() == consumeEventView.getId()) {
	...
	private boolean isInBoundsClickable(int x, int y, View child) {
		...
		Rect rect = new Rect(location[0], location[1], location[0] + child.getWidth(), location[1]
				+ child.getHeight());
		...
		if (mSelected != null) {
			...
			if (prevSelected.itemView.getParent() != null) {
				...
				final long duration = mCallback.getAnimationDuration(mRecyclerView, animationType,
						targetTranslateX - currentTranslateX, targetTranslateY -
								currentTranslateY);
	...
	public void setSwipActionInterface(SwipeAcitonInterface swipActionInterface) {
		this.mSwipActionInterface = swipActionInterface;
	}

	private float getSwipeWidth(ViewHolder holder) {
		if (mSwipActionInterface != null) {
			return mSwipActionInterface.getActionWidth(holder);
		}
	...
	private void moveIfNecessary(ViewHolder viewHolder) {
		...
		if (Math.abs(y - viewHolder.itemView.getTop()) < viewHolder.itemView.getHeight() *
				threshold
	...
	public static interface ViewDropHandler {
		...
		/**
		 * Called by the {@link ItemTouchHelperExtension} after a View is dropped over another
		 * View.
		 * <p>
	...
	public abstract static class Callback {
		...
		public static int makeSwipFlags(int dragFlags, int swipeFlags) {
			return makeFlag(ACTION_STATE_IDLE, swipeFlags | dragFlags) |
		...
		public View getItemFrontView(ViewHolder viewHolder) {
			...
			if (viewHolder.itemView instanceof ViewGroup && ((ViewGroup) viewHolder.itemView)
					.getChildCount() > 1) {
		...
		public void onMoved(final RecyclerView recyclerView,
                            final ViewHolder viewHolder, int fromPos, final ViewHolder target, int
									toPos, int x,
                            int y) {
		...
		public void onChildDrawOver(Canvas c, RecyclerView recyclerView,
                                    ViewHolder viewHolder,
                                    float dX, float dY, int actionState, boolean
											isCurrentlyActive) {
	...
	private class RecoverAnimation implements AnimatorListenerCompat {
		...
		public RecoverAnimation(ViewHolder viewHolder, int animationType,
								int actionState, float startDx, float startDy, float targetX,
								float targetY) {
			mActionState = actionState;
